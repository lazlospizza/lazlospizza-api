...
export const getArtistUnclaimedPayout = async () => {
  ...
  for (let i = 0; i < artists.length; i += 1) {
    try {
      ...
      const _allowedWithdrawalAmount = await contract.artistAllowedWithdrawalAmount(
        artist,
      )
      const allowedWithdrawalAmount = parseInt(
        _allowedWithdrawalAmount._hex,
        16,
      )
  ...
  for (let i = 0; i < payoutAddresses.length; i += 1) {
    const payouts = payoutHistory[payoutAddresses[i]]
      ?.map(async (payout: any) => {
        const isPaidOut = await contract.isPaidOutForBlock(
          payoutAddresses[i],
          payout.block,
        )
        if (isPaidOut) {
...
export const calculatePayouts = async () => {
  ...
  const _balance = await infuraProvider.getBalance(
    process.env.MAIN_CONTRACT_ADDRESS,
  )
  ...
  const unclaimedPayoutsTotal: number = unclaimedPayouts.reduce(
    (prev, current) =>
      prev +
      (current.payout?.payout_amount ? current.payout?.payout_amount / ETH : 0),
    0,
  )
  ...
  const rarityRewardPayouts = winningPizzas.map(
    ({ owner, tokenId, rarity }) => ({
      address: owner,
