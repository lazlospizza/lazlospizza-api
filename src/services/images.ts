...
const savePizzaLayerImage = async (url: string) => {
  const writer = fs.createWriteStream('temp/' + url.replace('https://lazlos-pizza.s3.amazonaws.com/pizza_layers/', ''));
  await axios({
    method: 'get',
    url: url,
    responseType: 'stream',
...
const tableClothImage = () => savePizzaLayerImage('https://lazlos-pizza.s3.amazonaws.com/pizza_layers/table_cloth.png');

const getIngredientLayerImg = async (tokenId: number) => savePizzaLayerImage(`https://lazlos-pizza.s3.amazonaws.com/pizza_layers/${tokenId}.png`);
...
export const pizzaImageFromTokenId = async (tokenId: number) => {
  ...
  await Promise.all(ingredientTokenIds.map(tokenId => getIngredientLayerImg(tokenId)));
